...
def stage_max_package_payload(repo_root: Path, payload_root: Path) -> int:
    ...
    help_files = [
        ...
        repo_root / "Tests" / "01-Score-Follower-Benchmarks" / "real" / "canticos.scofo",
        repo_root / "Tests" / "01-Score-Follower-Benchmarks" / "real" / "canticos.wav",
    ...
    return [
        IntegrationSpec(
            ...
            required=[
                ...
                Path("Tests/01-Score-Follower-Benchmarks/real/canticos.wav"),
                Path("Tests/01-Score-Follower-Benchmarks/real/canticos.scofo"),
            ...
            optional=[
                ...
                Path(
                    "build/Sources/Wrappers/Max/openscofo~.mxo/Contents/MacOS/openscofo~"
                ),
            ],
        ),
        IntegrationSpec(
...
def stage_component_payload(
    repo_root: Path, work_dir: Path, spec: IntegrationSpec
) -> Path:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build OpenScofo macOS component installer"
    )
    ...
    if not pkgbuild_bin or not productbuild_bin:
        print(
            "Error: pkgbuild and productbuild are required on macOS.", file=sys.stderr
        )
        return 1
    ...
    generate_distribution_xml(
        distribution_xml, args.package_name, args.version, built_components
    )
